...
class XpressWalletClient implements XpressWalletClientContract
{
    ...
    protected function request(string $method, string $uri, array $options = [], ?array $loggedPayload = null, int $attempt = 1): array
    {
        ...
        try {
            ...
            if ($span) {
                try { $span->setAttribute('http.status_code', $res->getStatusCode()); $span->end(); } catch (\Throwable) {}
            }
            if ($cacheKey) {
            ...
            if ($span) { try { $span->setAttribute('error', true); $span->setAttribute('error.message', $e->getMessage()); $span->end(); } catch (\Throwable) {} }
            if ($status === 401) {
